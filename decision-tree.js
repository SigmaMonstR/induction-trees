      ...
      console.log(key_list);
        ...
        if(eval("output.stats."+key_list[0]).n <= min_node ){
                ...
                //Calculate optimum split among variables
                    opt = candidateSplit(eval("output.data."+key_list[0]), inputs, "target");

                //Split based on best
                    nested = nodeSplit(eval("output.data."+key_list[0]),opt);

                if(nested.l.length ==0 || nested.r.length ==0){
                    key_list.splice(0,1);
                ...
                //Update statistical summary
                    ...
                    stat = {"l": nodeStats(nested[key_list2[0]],"target"),"r": nodeStats(nested[key_list2[1]],"target")}
